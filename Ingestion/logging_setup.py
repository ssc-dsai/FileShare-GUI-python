...
- Console: INFO+ (progress & important messages only – captured by Gradio)
"""
...
# Prefer absolute import so it works with runpy.run_path
try:
    from Ingestion.config_Ingestion import CREATE_PER_DOC_LOGS
except ImportError:
...
def setup_summary_logger(logs_path: Path, log_name: str = "zz_extraction_summary.log") -> logging.Logger:
    ...
    # Console handler: INFO+ (Gradio captures this)
    ...
    logger.info("Logging levels → File: DEBUG, Console: INFO")
    logger.info("=" * 80)
...
def setup_document_logger(file_path: str, logs_path: Path) -> logging.Logger | None:
    """
    Optional per-document logger (only if CREATE_PER_DOC_LOGS = True).
    """
    ...
    safe_name = Path(file_path).stem[:50].replace(" ", "_")
